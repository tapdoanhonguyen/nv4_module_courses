...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    if (empty($error)) {
        try {
            ...
            if (empty($row['id'])) {
                $sql = 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_coupons (title, code, type, discount, date_start, date_end, quantity, date_added, status) VALUES (:title, :code, :type, :discount, :date_start, :date_end, :quantity, ' . NV_CURRENTTIME . ', 1)';
                $data_insert = array();
                ...
                $stmt = $db->prepare('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_coupons SET title = :title, code = :code, type = :type, discount = :discount, date_start = :date_start, date_end = :date_end, quantity = :quantity WHERE id=' . $row['id']);
